            ...
            <main className="w-full py-4 space-y-6">
                <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                        <h1 className="text-3xl font-bold text-foreground">
                            {registeredOnly ? 'Зарегистрированные клиенты' : 'База клиентов'}
                        </h1>
                        <p className="text-sm text-muted-foreground mt-1">
                            {registeredOnly ? 'Заявки на карту и клиенты с картой, прошедшие регистрацию на сайте.' : 'Все клиенты с картами, включая тех, кто ещё не зарегистрировался на сайте.'}
                        </p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                        <Link href={registeredOnly ? '/admin/client-database' : '/admin/client-barcodes'}>
                            <Button variant="outline">{registeredOnly ? 'База клиентов' : 'Зарегистрированные клиенты'}</Button>
                        </Link>
                ...
                <section className="min-h-[70vh] rounded-lg border border-border bg-card p-6">
                    <h2 className="text-xl font-semibold mb-4">
                        {registeredOnly ? 'Зарегистрированные клиенты' : 'Все держатели карт'}{' '}
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-border text-left">
                                        ...
                                        <th className="pb-2 pr-4 font-medium">НДС</th>
                                        <th className="pb-2 pr-4 font-medium">Юр. адрес</th>
                                        <th className="pb-2 pr-4 font-medium">Адрес</th>
                                        <th className="pb-2 pr-4 font-medium">Банк / IBAN</th>
                                        <th className="pb-2 pr-4 font-medium">Перс. код</th>
                                        {!registeredOnly && <th className="pb-2 pr-4 font-medium">Регистрация</th>}
                                ...
                                <tbody className="divide-y divide-border">
                                    {cardHolders.map((holder) => {
                                        ...
                                        <tr key={holder.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                            ...
                                            <td className="py-2 pr-4"><Input className="min-w-36" value={edit.name} onChange={(e) => setClientEdits((prev) => ({ ...prev, [holder.id]: { ...edit, name: e.target.value } }))} placeholder="Не указано" /></td>
                                            ...
                                            <td className="py-2 pr-4 text-xs">{edit.customerType === 'company' ? holder.vatNumber || 'Не указан' : '—'}</td>
                                            <td className="min-w-48 py-2 pr-4 text-xs">{edit.customerType === 'company' ? holder.legalAddress || 'Не указан' : '—'}</td>
                                            <td className="min-w-48 py-2 pr-4 text-xs">{holder.address || 'Не указан'}</td>
                                            <td className="min-w-48 py-2 pr-4 text-xs">{holder.bankName || holder.iban ? <>{holder.bankName || 'Банк не указан'}{holder.iban && <span className="block font-mono">{holder.iban}</span>}</> : 'Не указан'}</td>
                                            <td className="py-2 pr-4 font-mono text-xs">{edit.customerType === 'individual' ? holder.personalCodeMasked ?? 'Не указан' : '—'}</td>
                                            {!registeredOnly && <td className="py-2 pr-4"><span className={holder.registered ? 'text-emerald-600' : 'text-muted-foreground'}>{holder.registered ? 'Зарегистрирован' : 'Не зарегистрирован'}</span></td>}
                                            <td className="py-2">
                                                {holder.bonusPoints}
