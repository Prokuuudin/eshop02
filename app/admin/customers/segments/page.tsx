...
export default function AdminCustomerSegmentsPage() {
  ...
  // Recipients for broadcast = current filter, no search constraint (ignore search for broadcast)
  ...
  const sendBroadcast = async () => {
    ...
    const confirmed = window.confirm(
      `Отправить письмо ${broadcastRecipients.length} получателям${activeTab !== 'Все' ? ` (${activeTab})` : ''}?`
    )
    if (!confirmed) return
              ...
              <div className="border-t border-indigo-100 dark:border-indigo-800 px-5 py-4 space-y-4">
                ...
                <div className="flex flex-wrap items-center gap-3 pt-1">
                  <Button
                    ...
                    disabled={bSending || !bSubject.trim() || !bBody.trim() || broadcastRecipients.length === 0}
                ...
                {filtered.map((c) => (
                  <tr key={c.email} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                    ...
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400">
                      {c.lastOrderDate ? c.lastOrderDate.toLocaleDateString('ru-RU') : '—'}
                    </td>
