...
type DeliveryFilter = 'all' | 'email' | 'app' | 'both' | 'disabled'
...
export default function AdminNotificationsSendPage(): React.ReactElement {
  ...
  const handleAttachmentUpload = async (files: FileList | null) => {
    ...
    try {
      ...
      for (const file of Array.from(files)) uploaded.push(await uploadAsset(file))
    ...
    <main className="w-full py-4 space-y-6 text-foreground">
        ...
        <section className="overflow-hidden rounded-xl border border-sky-200 bg-sky-50/70 dark:border-sky-900 dark:bg-sky-950/25" aria-labelledby="notification-help-title">
          <button
            ...
            className={`flex w-full items-start gap-3 px-4 py-3 text-left transition-colors hover:bg-sky-100/70 dark:hover:bg-sky-900/30 ${helpExpanded ? 'border-b border-sky-200 dark:border-sky-900' : ''}`}
          >
            ...
            <span className="mt-0.5 flex shrink-0 items-center gap-1 text-xs font-medium text-sky-800 dark:text-sky-200">
              {helpExpanded
          ...
          {helpExpanded && <div id="notification-help-content" className="grid gap-4 px-4 py-4 text-sm text-sky-950 dark:text-sky-100 md:grid-cols-2 xl:grid-cols-4">
            <div>
            ...
            <div>
              ...
              <p className="mt-1 text-xs leading-relaxed text-sky-800/80 dark:text-sky-200/75">
                {l('Можно загрузить до 3 изображений или PDF по 10 MB (всего до 20 MB).', 'Upload up to 3 images or PDFs of 10 MB each (20 MB total).', 'Var augšupielādēt līdz 3 attēliem vai PDF failiem pa 10 MB (kopā 20 MB).')}
              </p>
            </div>
            <div>
              <p className="font-semibold">4. {l('Проверьте доставку', 'Review delivery', 'Pārbaudiet piegādi')}</p>
              <p className="mt-1 text-xs leading-relaxed text-sky-800/80 dark:text-sky-200/75">
                {l('Канал выбирает клиент в своём кабинете. Он указан рядом с каждым получателем и не может быть изменён администратором.', 'Clients choose their channel in their account. It is shown beside each recipient and cannot be changed by an administrator.', 'Klients izvēlas kanālu savā kontā. Tas ir redzams pie katra saņēmēja, un administrators to nevar mainīt.')}
              </p>
            </div>
          </div>}
        </section>
          ...
          <section className="rounded-lg border border-border bg-card p-6 space-y-4">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium">
                {l('Канал доставки', 'Delivery channel', 'Piegādes kanāls')}
              </label>
              <Select
                value={deliveryFilter}
                onValueChange={(value) => { setDeliveryFilter(value as DeliveryFilter); setPage(0) }}
              >
                <SelectTrigger className="w-full">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">{l('Все клиенты', 'All clients', 'Visi klienti')}</SelectItem>
                  <SelectItem value="email">{l('Email', 'Email', 'E-pasts')}</SelectItem>
                  <SelectItem value="app">{l('Кабинет', 'Account', 'Konts')}</SelectItem>
                  <SelectItem value="both">{l('Email и кабинет', 'Email and account', 'E-pasts un konts')}</SelectItem>
                  <SelectItem value="disabled">{l('Все уведомления отключены', 'All notifications disabled', 'Visi paziņojumi izslēgti')}</SelectItem>
                </SelectContent>
              </Select>
            </div>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {u.notificationsSubscribed
                          ? l(`Канал: ${u.notificationChannel}`, `Channel: ${u.notificationChannel}`, `Kanāls: ${u.notificationChannel}`)
                          : l('Уведомления отключены клиентом', 'Notifications disabled by client', 'Klients ir izslēdzis paziņojumus')}
                      </p>
                    </div>
                  </label>
                ))
