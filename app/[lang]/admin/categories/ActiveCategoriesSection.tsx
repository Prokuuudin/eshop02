...
export default function ActiveCategoriesSection({ state }: { state: CategoriesState }): React.ReactElement {
 ...
 return <><section className="space-y-3">
          <h2 className="text-lg font-semibold text-foreground">
            {tl('admin.categories.existingCatalog', 'Ð¡ÑƒÑ‰ÐµÑÑ‚Ð²ÑƒÑŽÑ‰Ð¸Ðµ ÐºÐ°Ñ‚ÐµÐ³Ð¾Ñ€Ð¸Ð¸ ÐºÐ°Ñ‚Ð°Ð»Ð¾Ð³Ð°', 'Existing catalog categories', 'EsoÅ¡Äs kataloga kategorijas')}
          </h2>
          {loading ? (
            <div className="rounded-lg border border-gray-200 bg-white p-4 text-sm text-gray-600 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
              {tl('admin.categories.loading', 'Ð—Ð°Ð³Ñ€ÑƒÐ·ÐºÐ° ÐºÐ°Ñ‚ÐµÐ³Ð¾Ñ€Ð¸Ð¹...', 'Loading categories...', 'Ieladejam kategorijas...')}
            </div>
          ) : (
              ...
              <article key={category.id} className="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
                  ...
                  <div className="flex items-center gap-2">
                    <Button size="sm" onClick={() => void handleSaveCategory()} disabled={saving}>
                      {saving
                        ...
                        : tl('admin.categories.saveButton', 'Ð¡Ð¾Ñ…Ñ€Ð°Ð½Ð¸Ñ‚ÑŒ', 'Save', 'Saglabat')}
                    </Button>
                    <Button size="sm" variant="outline" onClick={() => handleResetCategoryChanges(category.id)} disabled={saving}>
                      {tl('admin.categories.resetButton', 'Ð¡Ð±Ñ€Ð¾ÑÐ¸Ñ‚ÑŒ', 'Reset', 'Atiestatit')}
                    </Button>
                    <Button size="sm" variant="destructive" onClick={() => void handleMoveCategoryToTrash(category.id)} disabled={saving}>
                      {tl('admin.categories.moveToTrashButton', 'Ð’ ÐºÐ¾Ñ€Ð·Ð¸Ð½Ñƒ', 'Move to trash', 'Uz grozu')}
                    </Button>
                  </div>
                </div>
                ...
                <div className="mt-4 rounded-md border border-dashed border-gray-300 p-3 dark:border-gray-700">
                  <p className="text-xs font-semibold text-gray-700 dark:text-gray-200">
                    {tl('admin.categories.subcategories', 'ÐŸÐ¾Ð´Ð¿ÑƒÐ½ÐºÑ‚Ñ‹', 'Subcategories', 'Apakskategorijas')} ({category.subcategories.length})
                  </p>
                      ...
                      <div key={`${category.id}-${subcategory.slug}`} className="rounded border border-gray-200 p-2 dark:border-gray-700">
                        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                          ...
                          <Button
                            ...
                            disabled={saving}
                          >
                            {tl('admin.categories.deleteButton', 'Ð£Ð´Ð°Ð»Ð¸Ñ‚ÑŒ', 'Delete', 'Dzest')}
                          </Button>
                        </div>
                        <div className="grid gap-2 md:grid-cols-4">
                          <AccessibleLabel className="text-xs">
                  ...
                  <div className="mt-3 flex justify-end">
                    <Button size="sm" onClick={() => void handleAddSubcategory(category.id)} disabled={saving}>
                      {saving
                        ? tl('admin.categories.saving', 'Ð¡Ð¾Ñ…Ñ€Ð°Ð½ÐµÐ½Ð¸Ðµ...', 'Saving...', 'Saglabasana...')
                        : tl('admin.categories.addSubButton', 'Ð”Ð¾Ð±Ð°Ð²Ð¸Ñ‚ÑŒ Ð¿Ð¾Ð´Ð¿ÑƒÐ½ÐºÑ‚', 'Add subcategory', 'Pievienot apakskategoriju')}
                    </Button>
                  </div>
                </div>
              </article>
