    ...
    return (
        <span
            className={`ml-1 text-[10px] ${
                active ? 'text-foreground' : 'text-gray-300 dark:text-gray-600'
            }`}
        >
            {active && dir === 'desc' ? '▼' : '▲'}
        </span>
...
export function InvitationPager({
    page,
    pageCount,
    total,
    setPage,
    ...
    return (
        <div className="flex flex-wrap items-center justify-between gap-2 text-sm text-muted-foreground">
            <span>
                {l(
                    `${start}–${end} из ${total.toLocaleString(locale)}`,
                    `${start}–${end} of ${total.toLocaleString(locale)}`,
                    `${start}–${end} no ${total.toLocaleString(locale)}`
                )}
            </span>
