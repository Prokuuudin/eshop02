...
export default function AdminBlogPage(): React.ReactElement {
  ...
  const { router, language, t, l, tl, locale, blogPosts, setBlogPosts, blogLoading, setBlogLoading, blogSaving, setBlogSaving, blogMessage, setBlogMessage, blogError, setBlogError, blogForm, setBlogForm, editingBlogId, setEditingBlogId, loadBlogPosts, handleBlogCreate, handleBlogDelete, handleStartEditBlog, handleCancelBlogEdit } = pageState
return (
    <AdminGate>
...
<div className="flex flex-wrap justify-between items-center gap-3 mb-8">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">{tl('admin.blog.subtitle', 'Создание, редактирование и удаление статей', 'Create, edit, and delete posts', 'Rakstu izveide, redigesana un dzesana')}</p>
            </div>
          ...
          <div className="bg-card rounded-lg border border-border p-6 mt-8">
              ...
              <Tabs defaultValue="base">
                ...
                <TabsContent value="base">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <label className="text-sm">
                      <span className="block text-muted-foreground mb-1">Категория</span>
                      <input
                    ...
                    <label className="text-sm md:col-span-2">
                      <span className="block text-muted-foreground mb-1">Заголовок</span>
                      <input
                    ...
                    <label className="text-sm md:col-span-2">
                      <span className="block text-muted-foreground mb-1">Краткое описание</span>
                      <textarea
                    ...
                    <label className="text-sm">
                      <span className="block text-muted-foreground mb-1">Автор</span>
                      <input
                    ...
                    <label className="text-sm">
                      <span className="block text-muted-foreground mb-1">Время чтения (мин)</span>
                      <input
                        type="number"
                        min={1}
                    ...
                    <label className="text-sm md:col-span-2">
                      <span className="block text-muted-foreground mb-1">Обложка (путь)</span>
                      <input
                    ...
                    <label className="text-sm md:col-span-2">
                      <span className="block text-muted-foreground mb-1">Legacy content (опционально)</span>
                      <textarea
                ...
                {(['en', 'lv'] as const).map((lang) => (
                  <TabsContent key={lang} value={lang}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <label className="text-sm md:col-span-2">
                        <span className="block text-muted-foreground mb-1">Заголовок ({lang})</span>
                        <input
                      ...
                      <label className="text-sm md:col-span-2">
                        <span className="block text-muted-foreground mb-1">Краткое описание ({lang})</span>
                        <textarea
                      ...
                      <label className="text-sm">
                        <span className="block text-muted-foreground mb-1">Автор ({lang})</span>
                        <input
                      ...
                      <label className="text-sm">
                        <span className="block text-muted-foreground mb-1">Категория ({lang})</span>
                        <input
                      ...
                      <label className="text-sm md:col-span-2">
                        <span className="block text-muted-foreground mb-1">Legacy content ({lang}, опционально)</span>
                        <textarea
            ...
            <div>
              ...
              {blogLoading ? (
                <p className="text-muted-foreground">{tl('admin.blog.loading', 'Загрузка...', 'Loading...', 'Ielade...')}</p>
              ) : blogPosts.length === 0 ? (
                <p className="text-muted-foreground">{tl('admin.blog.empty', 'Статей пока нет', 'No posts yet', 'Rakstu vel nav')}</p>
              ) : (
                <div className="space-y-2">
                  {blogPosts.map((post) => (
                    <div key={post.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 rounded border border-border p-3">
                      <div>
                        <p className="font-medium">{post.title}</p>
                        <p className="text-xs text-muted-foreground">/{post.slug} • {formatDate(post.createdAt, locale)}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <Button type="button" variant="outline" size="sm" onClick={() => handleStartEditBlog(post)}>
                          {tl('admin.blog.edit', 'Редактировать', 'Edit', 'Rediget')}
                        </Button>
                        <a href={`/blog/${post.slug}`} className="text-sm text-primary hover:underline" target="_blank" rel="noopener noreferrer">
