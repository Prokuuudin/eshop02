...
describe('GET /api/admin/analytics/abc', () => {
  it('returns a paginated page and full-dataset summary', async () => {
    queryRawMock.mockResolvedValueOnce([{
      rows: [{ id: 'p26', title: 'Mask', brand: 'Brand', qty: 2, revenue: 30, revenuePct: 0.1, cumPct: 0.9, grade: 'B', xyzGrade: 'Y', variationCoeff: 0.2 }],
      total: 31,
      summary: { A: { count: 8, revenue: 800 }, B: { count: 5, revenue: 150 } },
      matrix: { BY: { count: 5, revenue: 150 } },
    }])
    ...
    expect(body.matrix.BY.count).toBe(5)
  ...
  it('classifies a row by the cumulative share before that row', async () => {
    queryRawMock.mockResolvedValueOnce([{ rows: [], total: 0, summary: {}, matrix: {} }])
  ...
  it('includes zero-demand buckets and standard XYZ coefficient thresholds', async () => {
    queryRawMock.mockResolvedValueOnce([{ rows: [], total: 0, summary: {}, matrix: {} }])
    await GET(new NextRequest('http://localhost/api/admin/analytics/abc?xyz=X'))
    const sql = queryRawMock.mock.calls[0][0]
  ...
  it('exports the filtered report as UTF-8 CSV', async () => {
    queryRawMock.mockResolvedValueOnce([{ rows: [{ id: 'p1', title: 'Mask', brand: 'Brand', qty: 2, revenue: 30, revenuePct: 1, cumPct: 1, grade: 'A', xyzGrade: 'X', variationCoeff: 0.05 }], total: 1, summary: { A: { count: 1, revenue: 30 } }, matrix: { AX: { count: 1, revenue: 30 } } }])
