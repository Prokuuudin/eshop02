...
type Props = {
    brand: BrandConfigItem;
    state: Pick<State, 'tl' | 'updateBrandManufacturer' | 'updateBrandDistributor'>;
};
const fields: Array<{ field: LegalField; suffix: string; labels: [string, string, string] }> = [
    { field: 'name', suffix: 'Name', labels: ['Название', 'Name', 'Nosaukums'] },
    {
        field: 'address',
        suffix: 'Address',
        labels: ['Почтовый адрес', 'Postal address', 'Pasta adrese'],
    },
    { field: 'email', suffix: 'Email', labels: ['Электронная почта', 'Email', 'E-pasts'] },
];
...
export default function BrandLegalSection({ brand, state }: Props): React.ReactElement {
    ...
    const hasData = [brand.manufacturer, brand.distributor].some((party) =>
        Boolean(party?.name || party?.address || party?.email)
    );
                    ...
                    <AccordionContent className="px-4 pb-4 pt-2">
                        <div className="grid gap-4">
                            {(['manufacturer', 'distributor'] as const).map((party) => {
                                ...
                                return (
                                    <div key={party} className={`admin-brands__${party}-group`}>
                                        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                                            {heading}
                                        </p>
                                        <div className="admin-brands__legal-fields grid gap-2 md:grid-cols-3">
                                            {fields.map(({ field, suffix, labels }) => (
                                                <label key={field} className="text-xs">
                                                    <span className="mb-1 block text-muted-foreground">
                                                        {tl(
                                                            `admin.brands.field.${prefix}${suffix}`,
                                                            ...labels
                                                        )}
                                                    </span>
                                                    <Input
