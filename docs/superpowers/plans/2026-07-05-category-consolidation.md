...
- The bulk DB update is destructive-ish (rewrites `category` on ~6378 live rows backing the production site hairshoppro.lv.vercel.app) — it must run dry-run first, be reviewed, and only then applied with an explicit `--apply` flag. Do not run `--apply` without showing the dry-run report first.
