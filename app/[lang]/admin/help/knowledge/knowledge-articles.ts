...
export type KnowledgeLanguage = 'ru' | 'en' | 'lv';

export function getKnowledgeArticles(language: KnowledgeLanguage): KnowledgeArticle[] {
    const translations = language === 'en' ? articlesEn : language === 'lv' ? articlesLv : null;
    return articles.map((article, index) => ({ ...article, ...(translations?.[index] ?? {}) }));
}
