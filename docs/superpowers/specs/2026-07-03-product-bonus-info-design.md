...
- **1 балл = 1 цент** (`BONUS_POINT_VALUE_EUR = 0.01`, обновлено 2026-07-04; изначально
  было 1 балл = 1 €, но накопление было незаметным: €100 → 1 балл).
- Следствие: заказ €30 → +15 баллов, €100 → +50; списание 100 баллов = €1 скидки.
- Баллы целые (`User.bonusPoints Int`, схема БД не меняется).
...
## Движение баланса (добавлено 2026-07-04)

До этого баланс не менялся вообще: `adjustUserBonusPoints` звался только из админки,
а `/api/user/bonus` запрещает клиенту положительную дельту.

- Дебет (`bonusSpent`) и кредит (`bonusEarned`) применяются **сервером** в транзакции
  создания заказа (`createOrUpdateServerOrder`, рядом со списанием стока и promo
  usedCount) — только при создании, повторный POST того же заказа баланс не трогает.
- Значения к моменту записи уже пересчитаны и ограничены `recomputeOrderPricing`.
- Клиент после POST `/api/orders` вызывает `syncBonusBalanceFromServer()` (lib/auth) —
  подтягивает свежий баланс из GET `/api/user/bonus` в localStorage.
- Гости баллы не копят (заказ без `userId`).
...
2. Начисление (обновлено 2026-07-04 — живое число за единицу):
   - `perUnitPoints = calcOrderBonus([{price, qty: 1, bonusRate}])` > 0 →
     «Начислится после заказа: +X баллов» (`checkout.bonus.willEarn`);
   - иначе (цена < ~€1) → «Начисляется 0.5% от суммы каждого заказа»
