...
- Канал доставки задаётся через существующий пайплайн `UserNotification` —
  новый механизм доставки не добавляем. На практике это значит: запись
  всегда попадает в приложение, и дополнительно дублируется на email,
  если у пользователя уже включён `channel: email/both` в его настройках
  уведомлений (тот же пайплайн, что и у остальных типов уведомлений).
  Проверено ревью 2026-08-20: осознанно оставлено как есть — это
  существующий opt-in пользователя, а не новая утечка приватности.
