...
export default function AdminClientBarcodesPage(): React.ReactElement {
    ...
    const {
        ...
        handleApproveNoCardRequest,
        handleRejectNoCardRequest,
    } = pageState;
            ...
            <main className="w-full py-4 space-y-6">
                <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                        <h1 className="text-3xl font-bold text-foreground">{registeredOnly ? l('Зарегистрированные клиенты', 'Registered customers', 'Reģistrētie klienti') : l('База клиентов', 'Customer database', 'Klientu datubāze')}</h1>
                        <p className="text-sm text-muted-foreground mt-1">
                            {registeredOnly
                                ...
                                : l(
                                      'Все клиенты с картами, включая тех, кто ещё не зарегистрировался на сайте.',
                                      'All cardholders, including those who have not registered on the website yet.',
                                      'Visi karšu īpašnieki, tostarp tie, kuri vēl nav reģistrējušies vietnē.'
                                  )}
                        </p>
                    </div>
                    ...
                    <section className="rounded-lg border border-amber-200 dark:border-amber-800 bg-card p-6">
                        <div className="mb-4">
                            <h2 className="text-xl font-semibold text-foreground">
                                ...
                                <span className="ml-1 rounded-full bg-amber-100 dark:bg-amber-900/40 px-2 py-0.5 text-sm font-medium text-amber-800 dark:text-amber-300">{noCardRequests.length}</span>
                            </h2>
                            <p className="text-sm text-muted-foreground mt-1">
                                {l(
                                    'Проверьте заявку, выберите тип клиента и выдайте свободный номер карты.',
                                    'Review the application, select the customer type, and assign an available card number.',
                                    'Pārbaudiet pieteikumu, izvēlieties klienta veidu un piešķiriet brīvu kartes numuru.'
                                )}
                            </p>
                        </div>
                                        ...
                                        <div key={req.id} className="rounded-lg border border-border p-4 space-y-3">
                                            <div className="flex flex-wrap items-start justify-between gap-3">
                                                <div className="space-y-1 min-w-0">
                                                    <p className="font-semibold text-foreground">{req.name || req.email}</p>
                                                    <p className="text-sm text-muted-foreground">Email: {req.email}</p>
                                                    {req.phone && (
                                                        <p className="text-sm text-muted-foreground">
                                                            {l('Телефон', 'Phone', 'Tālrunis')}: {req.phone}
                                                        </p>
                                                    )}
                                                    <p className="text-sm text-muted-foreground">{new Date(req.requestedAt).toLocaleString(locale)}</p>
                                                </div>
                                                {req.certificateName && (
                                                    <a
                                            ...
                                            {req.message && <p className="rounded bg-muted px-3 py-2 text-sm text-muted-foreground italic">«{req.message}»</p>}
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                                                <label htmlFor={`customer-type-${req.id}`} className="text-sm">
                                                    ...
                                                    >
                                                        <SelectTrigger id={`customer-type-${req.id}`}>
                                                            <SelectValue />
                                                        </SelectTrigger>
                                                        <SelectContent>
                                                            <SelectItem value="individual">{l('Физлицо', 'Individual', 'Privātpersona')}</SelectItem>
                                                            <SelectItem value="company">{l('Юрлицо', 'Company', 'Uzņēmums')}</SelectItem>
                                                        </SelectContent>
                                                    </Select>
                                                </label>
                                                ...
                                                {draft.customerType === 'company' && (
                                                    <label htmlFor={`registration-number-${req.id}`} className="text-sm">
                                                        ...
                                                        <Input
                                                            ...
                                                            placeholder={l('11 цифр', '11 digits', '11 cipari')}
                                                            inputMode="numeric"
                                                        />
                                                    </label>
                                                )}
                                                <label htmlFor={`card-number-${req.id}`} className="text-sm">
                                                    ...
                                                    <div className="flex gap-2">
                                                        <Input
                                                            ...
                                                            className="font-mono"
                                                        />
                                                        <Button type="button" variant="outline" size="sm" className="shrink-0" onClick={() => void regenerateCardNumber(req.id)}>
                                                            ↺
                                                        </Button>
                                                    </div>
                                                </label>
                                            ...
                                            <div className="pt-1">
                                                ...
                                                <Textarea
                                                    ...
                                                    rows={2}
                                                    placeholder={l(
                                                        'Например: предоставленный документ не является действующим сертификатом...',
                                                        'For example: the submitted document is not a valid certificate...',
                                                        'Piemēram: iesniegtais dokuments nav derīgs sertifikāts...'
                                                    )}
                                                    className="w-full resize-none text-sm"
                                                />
                                            </div>
                                            <div className="flex gap-2">
                                                <Button size="sm" disabled={emailBusy[req.id]} onClick={() => handleApproveNoCardRequest(req.id, req.email)}>
                                                    {emailBusy[req.id] ? l('Отправка...', 'Sending...', 'Nosūta...') : l('Выдать карту и уведомить', 'Issue card and notify', 'Izsniegt karti un paziņot')}
                                                </Button>
                                                <Button size="sm" variant="outline" disabled={emailBusy[req.id]} onClick={() => handleRejectNoCardRequest(req.id, req.email, req.name || req.email)}>
                ...
                <section className="min-h-[70vh] rounded-lg border border-border bg-card p-6">
                    <h2 className="text-xl font-semibold mb-4">
                        ...
                        <span className="text-muted-foreground font-normal text-base">{cardHoldersTotal === 0 ? '0' : `${cardHoldersPage * 50 + 1}–${Math.min((cardHoldersPage + 1) * 50, cardHoldersTotal)} / ${cardHoldersTotal}`}</span>
                    </h2>
                        ...
                        <div className="relative min-w-64 flex-1">
                            ...
                            <Input
                                ...
                                onChange={(e) => {
                                    ...
                                    setCardHoldersPage(0);
                                }}
                                placeholder={l(
                                    'Поиск по карте, имени, email, телефону или рег. номеру...',
                                    'Search by card, name, email, phone, or registration number...',
                                    'Meklēt pēc kartes, vārda, e-pasta, tālruņa vai reģistrācijas numura...'
                                )}
                                className="h-9 pl-9"
                            />
                        </div>
                    ...
                    {cardHoldersLoading && cardHolders.length === 0 ? (
                        ...
                        <p className="text-sm text-muted-foreground py-4 text-center">
                            {search.trim()
                                ? l(`Ничего не найдено по запросу «${search}»`, `Nothing found for “${search}”`, `Vaicājumam “${search}” nekas nav atrasts`)
                                : l('Держателей карт пока нет.', 'There are no cardholders yet.', 'Karšu īpašnieku vēl nav.')}
                        </p>
                    ) : (
                                            ...
                                            <tr key={holder.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                                ...
                                                <td className="py-2 pr-4">
                                                    <Input
                                                        ...
                                                        placeholder={l('Не указано', 'Not specified', 'Nav norādīts')}
                                                    />
                                                </td>
                                                <td className="py-2 pr-4">
                                                    <Input
                                                        ...
                                                        placeholder={tl('admin.clientBarcodes.emailMissing', 'Не указано', 'Not specified', 'Nav norādīts')}
                                                    />
                                                </td>
                                                <td className="py-2 pr-4">
                                                    <Input
                                                        ...
                                                        placeholder={tl('admin.clientBarcodes.phoneMissing', 'Не указан', 'Not specified', 'Nav norādīts')}
                                                    />
                                                </td>
                                                <td className="py-2 pr-4">
                                                    <Select
                                                        value={edit.customerType}
                                                        onValueChange={(value: 'individual' | 'company') =>
                                                            setClientEdits((prev) => ({
                                                                ...prev,
                                                                [holder.id]: {
                                                ...
                                                <td className="py-2 pr-4">
                                                    <Input
                                                        ...
                                                        disabled={edit.customerType !== 'company'}
                                                        value={edit.customerType === 'company' ? edit.registrationNumber : ''}
                                                        onChange={(e) =>
                                                            setClientEdits((prev) => ({
                                                                ...prev,
                                                                [holder.id]: {
                                                ...
                                                <td className="min-w-48 py-2 pr-4 text-xs">
                                                    {holder.bankName || holder.iban ? (
                                                        <>
                                                            {holder.bankName || l('Банк не указан', 'Bank not specified', 'Banka nav norādīta')}
                                                            {holder.iban && <span className="block font-mono">{holder.iban}</span>}
                                                        </>
                                                    ) : (
                                                ...
                                                {!registeredOnly && (
                                                    <td className="py-2 pr-4">
                                                        <span className={holder.registered ? 'text-emerald-600' : 'text-muted-foreground'}>
                                                            {holder.registered ? l('Зарегистрирован', 'Registered', 'Reģistrēts') : l('Не зарегистрирован', 'Not registered', 'Nav reģistrēts')}
                                                        </span>
                                                    </td>
                                                )}
                                                <td className="py-2">
                                                    {holder.bonusPoints}
                                                    <span className="ml-1 text-xs text-muted-foreground">({formatEuro(pointsToEuros(holder.bonusPoints ?? 0), locale)})</span>
                                                </td>
                                                <td className="py-2 pl-2">
                                                    <Button size="sm" variant="outline" disabled={clientSaveBusy === holder.id} onClick={() => void handleSaveClientDetails(holder)}>
                                                        {clientSaveBusy === holder.id ? l('Сохранение…', 'Saving…', 'Saglabā…') : l('Сохранить', 'Save', 'Saglabāt')}
                                                    </Button>
                                                </td>
                                            </tr>
                                        );
                    ...
                    {cardHoldersTotal > 50 && (
                        <div className="mt-4 flex items-center justify-end gap-3 text-sm">
                            <Button size="sm" variant="outline" disabled={cardHoldersLoading || cardHoldersPage === 0} onClick={() => setCardHoldersPage((page) => Math.max(0, page - 1))}>
                                {l('Назад', 'Previous', 'Atpakaļ')}
                            </Button>
                            <span>
                                {cardHoldersPage + 1} / {Math.ceil(cardHoldersTotal / 50)}
                            </span>
                            <Button size="sm" variant="outline" disabled={cardHoldersLoading || (cardHoldersPage + 1) * 50 >= cardHoldersTotal} onClick={() => setCardHoldersPage((page) => page + 1)}>
                                {l('Далее', 'Next', 'Tālāk')}
                            </Button>
                        </div>
                    )}
