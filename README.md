...
| `FIRST_LOGIN_PASSWORD` | yes | Shared welcome password for a new B2B team member joining an existing company card (must change on first login). Individual cardholders activate via phone-last-4/email verification instead, not this password. Server-only — never prefix this `NEXT_PUBLIC_`, or it leaks into the client JS bundle. |
