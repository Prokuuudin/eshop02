...
Poll `https://hairshoppro.lv.vercel.app/api/products/bestsellers` the same way as Task 8 Step 3 (real number, not a string) — this is the "new code is live" signal for this change (there's no CSP-header-style marker like the A1 deploy check, since this change doesn't touch `next.config.js`).
