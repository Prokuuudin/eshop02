...
export function ShipmentForm({ order }: { order: Order }): React.ReactElement {
  ...
  const [carrier, setCarrier] = useState(order.shipmentCarrier ?? (order.deliveryMethod === 'venipak' ? 'venipak' : 'omniva'))
  ...
  return <div className="w-full">
    ...
    {open && <div className="mt-3 grid gap-3 rounded border p-3 sm:grid-cols-2">
      <label>{l('Перевозчик', 'Carrier', 'Pārvadātājs')}<select className="block rounded border bg-card p-2" value={carrier} onChange={e => setCarrier(e.target.value)}><option value="omniva">Omniva</option><option value="venipak">Venipak</option><option value="dpd">DPD</option><option value="other">{l('Другой', 'Other', 'Cits')}</option></select></label>
