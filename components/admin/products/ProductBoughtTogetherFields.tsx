...
const ProductBoughtTogetherFields: React.FC = () => {
    ...
    const { fields, append, remove } = useFieldArray({ control, name: 'oftenBoughtTogether' as never });
            ...
            <div className="flex flex-col gap-2">
                {(fields as { id: string }[]).map((field, idx) => (
                    <div key={field.id} className="flex gap-2">
                        <Input
                            placeholder="ID товара (например: p3)"
                            {...register(`oftenBoughtTogether.${idx}` as const)}
                        />
