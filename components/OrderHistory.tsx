    ...
    <Card className="p-6 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 col-span-2">
          ...
          {monthlyData.map((month, idx) => {
            ...
            return (
              <div
                ...
                title={`${month.month}: ${month.count} заказов, ${formatEuro(month.revenue, locale)}`}
              >
      ...
      <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
        <div className="grid grid-cols-3 gap-4">
          <div>
            <p className="text-xs text-gray-600 dark:text-gray-400">Всего месяцев</p>
            <p className="text-lg font-bold text-gray-900 dark:text-gray-100">{monthlyData.length}</p>
          </div>
          <div>
            <p className="text-xs text-gray-600 dark:text-gray-400">Средник заказов/месяц</p>
            <p className="text-lg font-bold text-gray-900 dark:text-gray-100">
              {(analytics.totalOrders / monthlyData.length).toFixed(1)}
            </p>
          </div>
          <div>
            <p className="text-xs text-gray-600 dark:text-gray-400">Макс. в месяц</p>
            <p className="text-lg font-bold text-gray-900 dark:text-gray-100">
