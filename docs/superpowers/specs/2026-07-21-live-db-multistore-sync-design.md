...
Спроектировать: топологию чтения/записи между eshop02 и hairshop_p34s, механизм разделения «локальная правка админа» vs «значение из общей базы» без breaking-изменений живой Neon-схемы, контракт write-back при заказе, безопасный поэтапный rollout.
...
eshop02 остаётся один магазин (не multi-tenant внутри себя — nopCommerce-мультистор существует на стороне живой БД, не у нас, заводить свой `storeId` по всей схеме eshop02 не нужно, это over-engineering под несуществующую задачу).
...
- Concurrent-run guard (`SyncRun.status==='running'`, stale >30 мин) продолжает защищать только от пересечения собственных прогонов eshop02. Он не видит и не может видеть синк-активность 2 других магазинов — это не проблема, т.к. наш путь чтения read-only относительно живой БД и идемпотентен.
...
`Product.stock` в Neon — кеш для витрины, обновляемый read-pull раз в час, **не** источник истины для гейта на checkout. Поэтому optimistic-lock на нём не нужен: гонки между собственными заказами eshop02 (и заказами двух других магазинов) разрешает атомарный decrement на стороне источника, не Neon.
...
4. Backup/restore и мониторинг на стороне живой БД — предусловие перед go-live, вне кода eshop02.
5. Кто выдаёт креды/API-key и как они попадают в Vercel env.

## Явно не делаем сейчас (YAGNI)

- Свой `storeId`/tenant-scoping по схеме eshop02 — multi-store не наша забота, она уже на стороне живой БД.
