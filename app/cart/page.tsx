...
export default function CartPage() {
  ...
  if (items.length === 0) {
    return (
      <main className="w-full px-4 py-12 text-gray-900 dark:text-gray-100">
        <h1 className="cart__title text-3xl font-bold mb-8 text-gray-900 dark:text-gray-100">{t('cart.title')}</h1>
  ...
  return (
    <main className="w-full px-4 py-8 text-gray-900 dark:text-gray-100">
      <h1 className="cart__title text-3xl font-bold mb-8 text-gray-900 dark:text-gray-100">{t('cart.title')} ({items.length})</h1>

      <div className="mb-4 flex flex-wrap items-center gap-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 text-sm">
      ...
      <div className="cart__layout grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="cart__items lg:col-span-2">
          <div className="space-y-4">
            {items.map((item) => {
              ...
              return (
                <div key={item.id} className="cart__item p-4 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 flex gap-4">
                  <div className="pt-1">
                    <Checkbox
                      checked={isSelected}
                  ...
                  <div className="cart__item-image w-24 h-24 flex-shrink-0 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden relative">
                    <Image src={item.image} alt={localizedTitle} fill className="object-cover" />
                  </div>

                  <div className="cart__item-info flex-1">
                    <Link href={`/product/${item.id}`} className="font-medium hover:text-indigo-600">
                      {localizedTitle}
                    </Link>
                    <div className="text-sm text-gray-600 dark:text-gray-300 mt-1">{item.brand}</div>
                    <div className="text-indigo-600 font-bold mt-2">{formatCurrency(unitPrice)}</div>
                  </div>
                  ...
                  <div className="cart__item-controls flex flex-col items-end justify-between">
                    <ConfirmActionDialog
                      ...
                      trigger={<button className="text-red-600 text-sm">{t('cart.remove')}</button>}
                    />
                    ...
                    {minQuantity > 1 && (
                      <div className="text-xs text-gray-500 dark:text-gray-300 mt-1">{t('common.min')} {minQuantity} {t('product.pcs')}</div>
                    )}

                    <div className="font-bold">{formatCurrency(unitPrice * item.quantity)}</div>
                  </div>
                </div>
              )
            })}
          ...
          <ConfirmActionDialog
            ...
            trigger={<Button variant="outline" className="mt-6">{t('cart.clear')}</Button>}
          />
        </div>
        ...
        <aside className="cart__summary sticky top-20 h-fit">
          <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            <h2 className="font-bold text-lg mb-4 text-gray-900 dark:text-gray-100">{t('cart.total')}</h2>

            {!wholesaleGuard.isMinimumReached && selectedItemIds.length > 0 && (
              <WholesaleMinimumAlert
                className="mb-4 rounded border border-amber-300 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/30 p-3 text-sm text-amber-800 dark:text-amber-200"
            ...
            <div className="space-y-3 text-sm border-b border-gray-200 dark:border-gray-700 pb-4 text-gray-700 dark:text-gray-300">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-900 dark:text-gray-100">{formatCurrency(deliveryFee)}</span>
              </div>
            </div>
