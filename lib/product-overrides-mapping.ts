...
export function mapDbToProduct(p: PrismaProduct): Product {
    return {
        ...
        minOrderQuantities: (p.minOrderQuantities ?? undefined) as
            | Record<string, number>
            | undefined,
        technicalSpecs: (p.technicalSpecs ?? undefined) as Record<string, string> | undefined,
        bulkPricingTiers: (p.bulkPricingTiers ?? undefined) as
            | Array<{ quantity: number; pricePerUnit: number }>
            | undefined,
        demoVideo: (p.demoVideo ?? undefined) as
            | Array<{ src: string; poster?: string }>
            | undefined,
        distributorName: (p.distributorName ?? undefined) as
            | { ru: string; en: string; lv: string }
            | undefined,
        distributorAddress: (p.distributorAddress ?? undefined) as
            | { ru: string; en: string; lv: string }
            | undefined,
        sku: p.sku ?? undefined,
...
function mapProductToDbCreateData(p: Product, isCustom = false) {
    return {
        id: p.id,
        title: p.title,
...
export function mapProductToDbCreate(
    p: Product,
    isCustom = false
): ReturnType<typeof mapProductToDbCreateData> {
    return mapProductToDbCreateData(p, isCustom);
}
