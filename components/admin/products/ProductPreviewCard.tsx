        ...
        <Card className="product-card px-3 py-2 min-h-[370px] flex flex-col relative min-w-0 bg-card border border-border text-foreground group shadow-sm">
            <div className="product-card__header flex items-center justify-between gap-2 mb-1 min-w-0">
                ...
                <div className="relative z-10 shrink-0">
                    <button
                        type="button"
                        aria-label={t('wishlist.addAria')}
                        className="inline-flex items-center justify-center rounded-full border border-gray-200 p-2 text-[#0088C4] shadow-sm transition bg-white/95 hover:border-pink-300 hover:text-pink-600 dark:border-gray-700 dark:bg-gray-900/95 dark:text-[#0088C4] dark:hover:border-pink-500 dark:hover:text-pink-400"
                        tabIndex={-1}
                    >
                        <svg
                            className="h-5 w-5"
            ...
            <div className="product-card__body mt-2 flex-1 flex flex-col min-w-0">
                <div className="product-card__title text-sm font-medium">
                    {stripBrandPrefix(title, brand ?? '')}
                </div>
                                ...
                                {firstTier && (
                                    <div className="text-xs text-emerald-700 dark:text-emerald-300 mt-1">
                                        {t('product.bulkTierPrice', undefined, {
                                            ...
                                            price: formatEuro(firstTier.pricePerUnit, locale),
                                        })}
                                    </div>
                                )}
                            </>
                ...
                <div className="product-card__actions relative z-10 mt-2 w-full space-y-2">
                    {!isOutOfStock && (
                        <div className="add-to-cart__combo flex items-stretch w-full h-9 rounded-md overflow-hidden text-white text-sm font-medium shadow divide-x divide-white/20 bg-indigo-600">
                            <button
                                type="button"
                                className="add-to-cart__minus w-9 shrink-0 flex items-center justify-center text-lg"
                                tabIndex={-1}
                            >
                                −
                            </button>
                            <button
                                type="button"
                                className="add-to-cart__button flex-1 min-w-0 px-1 flex items-center justify-center"
                                tabIndex={-1}
                            >
                                <span className="truncate">{t('product.addToCart')} (1)</span>
                            </button>
                            <button
                                type="button"
                                className="add-to-cart__plus w-9 shrink-0 flex items-center justify-center text-lg"
                                tabIndex={-1}
                            >
                                +
                            </button>
