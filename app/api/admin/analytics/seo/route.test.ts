...
describe('GET /api/admin/analytics/seo', () => {
  it('paginates and filters in SQL while passing overrides to the query', async () => {
    ...
    expect(query.strings.join(' ')).toContain('WHERE NOT p."isDeleted" AND p."isActive"')
    ...
    expect(query.strings.join(' ')).toContain('NOT "hasMetaTitle"')
    expect(query.strings.join(' ')).toContain("concat(title, ' | Hairshop-Pro')")
    expect(query.strings.join(' ')).toContain("concat(brand, ' — ', title)")
    expect(query.strings.join(' ')).toContain('validMetaTitleLength')
    expect(query.strings.join(' ')).toContain('duplicateMeta')
    expect(query.strings.join(' ')).toContain("'all', (SELECT COUNT(*)::int FROM searched WHERE NOT \"hasMetaTitle\"")
