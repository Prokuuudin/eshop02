...
<div className="space-y-3">
              ...
              <div className="flex items-start px-5 py-4 hover:bg-black/[.02] dark:hover:bg-white/[.02] transition-colors">
                ...
                <div className="text-right shrink-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {order.items.length} {order.items.length === 1 ? 'Ñ‚Ð¾Ð²Ð°Ñ€' : order.items.length < 5 ? 'Ñ‚Ð¾Ð²Ð°Ñ€Ð°' : 'Ñ‚Ð¾Ð²Ð°Ñ€Ð¾Ð²'}
                  </p>
                </div>
                </button>
              </div>
                ...
                <div className="border-t border-border px-5 py-5 space-y-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {!['shipped', 'delivered', 'cancelled'].includes(status) && (
                      <button
                        ...
                        className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-colors ${editingOrderId === order.id ? 'border-amber-400 bg-amber-50 text-amber-700 dark:border-amber-700 dark:bg-amber-900/20 dark:text-amber-300' : 'border-primary/50 dark:border-primary/50 text-primary dark:text-primary hover:bg-primary/5 dark:hover:bg-primary/10'}`}
                      >
                        {editingOrderId === order.id ? 'ÐžÑ‚Ð¼ÐµÐ½Ð¸Ñ‚ÑŒ Ð¿Ñ€Ð°Ð²ÐºÑƒ' : 'âœ Ð ÐµÐ´Ð°ÐºÑ‚Ð¸Ñ€Ð¾Ð²Ð°Ñ‚ÑŒ'}
                      </button>
                    )}
                    <a
                  ...
                  {editingOrderId === order.id && (
                    <div className="rounded-xl border-2 border-primary/30 dark:border-primary/50 bg-primary/5/30 dark:bg-primary/20/10 p-4 space-y-5">
                      <p className="text-sm font-semibold text-primary dark:text-primary/60">Ð ÐµÐ´Ð°ÐºÑ‚Ð¸Ñ€Ð¾Ð²Ð°Ð½Ð¸Ðµ Ð·Ð°ÐºÐ°Ð·Ð°</p>

                      {/* Address */}
                      <div className="space-y-2">
                        <p className="text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">ÐÐ´Ñ€ÐµÑ Ð´Ð¾ÑÑ‚Ð°Ð²ÐºÐ¸</p>
                      ...
                      {/* Delivery method */}
                      <div className="space-y-2">
                        <p className="text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">Ð¡Ð¿Ð¾ÑÐ¾Ð± Ð´Ð¾ÑÑ‚Ð°Ð²ÐºÐ¸</p>
                        <div className="flex flex-wrap gap-2">
                          {(['pickup', 'courier', 'post'] as const).map((dm) => (
                            <button key={dm} type="button"
                              onClick={() => setEditDelivery(dm)}
                              className={`rounded-lg border px-3 py-1.5 text-xs font-medium transition-colors ${editDelivery === dm ? 'border-primary/70 bg-primary/10 text-primary dark:border-primary dark:bg-primary/40 dark:text-primary/60' : 'border-border text-muted-foreground hover:border-gray-400'}`}
                            >
                              {DELIVERY_LABELS[dm]} {EDIT_DELIVERY_COSTS[dm] === 0 ? '(Ð±ÐµÑÐ¿Ð»Ð°Ñ‚Ð½Ð¾)' : `(â‚¬${EDIT_DELIVERY_COSTS[dm]})`}
                            </button>
                          ))}
                        </div>
                      </div>
                      ...
                      {/* Items */}
                      <div className="space-y-2">
                        <p className="text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">ÐŸÐ¾Ð·Ð¸Ñ†Ð¸Ð¸ Ð·Ð°ÐºÐ°Ð·Ð°</p>
                        <div className="rounded-lg border border-border divide-y divide-gray-100 dark:divide-gray-800 bg-card">
                          {editItems.map((item) => (
                            <div key={item.lineKey} className="flex items-center gap-3 px-3 py-2.5">
                              ...
                              <span className="text-xs text-gray-400 shrink-0">â‚¬{item.price.toFixed(2)}</span>
                              <div className="flex items-center gap-1 shrink-0">
                                <button type="button" onClick={() => editUpdateQty(item.lineKey, item.quantity - 1)} className="h-6 w-6 rounded border border-border text-muted-foreground hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center justify-center text-base leading-none">âˆ’</button>
                              ...
                              <span className="text-sm font-medium text-foreground w-16 text-right tabular-nums shrink-0">â‚¬{(item.price * item.quantity).toFixed(2)}</span>
                              <button type="button" onClick={() => editUpdateQty(item.lineKey, 0)} className="text-gray-300 hover:text-red-500 dark:text-gray-600 dark:hover:text-red-400 text-lg leading-none shrink-0">Ã—</button>
                            </div>
                          ))}
                        </div>
                              ...
                              {editProductResults.map((p) => (
                                <button key={p.id} type="button" onClick={() => editAddProduct(p)}
                                  ...
                                  <div className="min-w-0 flex-1">
                                    ...
                                    <p className="text-xs text-gray-400">{p.brand}{p.sku ? ` Â· ${p.sku}` : ''}</p>
                                  </div>
                                  <span className="text-sm font-semibold text-foreground shrink-0">â‚¬{p.price.toFixed(2)}</span>
                                </button>
                              ))}
                            </div>
                          )}
                        ...
                        return (
                          <div className="flex justify-end">
                            <div className="text-sm space-y-1 min-w-[220px]">
                              <div className="flex justify-between gap-4 text-muted-foreground"><span>Ð¢Ð¾Ð²Ð°Ñ€Ñ‹</span><span className="tabular-nums">â‚¬{newSub.toFixed(2)}</span></div>
                              {newDisc > 0 && <div className="flex justify-between gap-4 text-emerald-600 dark:text-emerald-400"><span>Ð¡ÐºÐ¸Ð´ÐºÐ°</span><span className="tabular-nums">âˆ’â‚¬{newDisc.toFixed(2)}</span></div>}
                              <div className="flex justify-between gap-4 text-muted-foreground"><span>Ð”Ð¾ÑÑ‚Ð°Ð²ÐºÐ°</span><span className="tabular-nums">{newDel === 0 ? 'Ð‘ÐµÑÐ¿Ð»Ð°Ñ‚Ð½Ð¾' : `â‚¬${newDel.toFixed(2)}`}</span></div>
                              <div className="flex justify-between gap-4 font-bold text-base text-foreground pt-1 border-t border-border"><span>Ð˜Ñ‚Ð¾Ð³Ð¾</span><span className="tabular-nums">â‚¬{newTotal.toFixed(2)}</span></div>
                            </div>
                          </div>
                        )
                      })()}
                      ...
                      <div className="flex gap-2 pt-1">
                        ...
                        >
                          {editSaving ? 'Ð¡Ð¾Ñ…Ñ€Ð°Ð½ÐµÐ½Ð¸Ðµ...' : 'Ð¡Ð¾Ñ…Ñ€Ð°Ð½Ð¸Ñ‚ÑŒ Ð¸Ð·Ð¼ÐµÐ½ÐµÐ½Ð¸Ñ'}
                        </button>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="rounded-lg border border-border p-4 space-y-2">
                      <p className="text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">ÐšÐ»Ð¸ÐµÐ½Ñ‚</p>
                    ...
                    {/* Delivery */}
                    <div className="rounded-lg border border-border p-4 space-y-2">
                      <p className="text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">Ð”Ð¾ÑÑ‚Ð°Ð²ÐºÐ°</p>
                      <p className="text-sm font-medium text-foreground">{DELIVERY_LABELS[order.deliveryMethod] ?? order.deliveryMethod}</p>
                      <div className="text-sm text-gray-700 dark:text-gray-300 space-y-0.5">
                        <p>{order.address}</p>
                        {order.postalCode && <p>Ð˜Ð½Ð´ÐµÐºÑ: {order.postalCode}</p>}
                        <p>{order.city}</p>
                      </div>
                    </div>
                    ...
                    {/* Payment */}
                    <div className="rounded-lg border border-border p-4 space-y-2">
                      <p className="text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">ÐžÐ¿Ð»Ð°Ñ‚Ð°</p>
                      ...
                      {order.paymentProvider && (
                        <p className="text-sm text-muted-foreground">ÐŸÑ€Ð¾Ð²Ð°Ð¹Ð´ÐµÑ€: <span className="text-foreground font-medium">{order.paymentProvider}</span></p>
                      )}
                  ...
                  {/* Items */}
                  <div>
                    <p className="text-sm font-semibold text-foreground mb-2">Ð¡Ð¾ÑÑ‚Ð°Ð² Ð·Ð°ÐºÐ°Ð·Ð°</p>
                    <div className="rounded-lg border border-border divide-y divide-gray-200 dark:divide-gray-700">
                      {order.items.map((item) => (
                        <div key={item.lineKey} className="flex items-center gap-3 px-3 py-2.5">
                          ...
                          <div className="text-right shrink-0">
                            <p className="text-xs text-muted-foreground">
                              {item.quantity} ÑˆÑ‚ Ã— {formatEuro(item.price, locale)}
                            </p>
                  ...
                  <div className="flex justify-end">
                    <div className="text-sm space-y-1.5 min-w-[260px]">
                      <div className="flex justify-between gap-6">
                        <span className="text-muted-foreground">Ð¡ÑƒÐ¼Ð¼Ð° Ð·Ð° Ñ‚Ð¾Ð²Ð°Ñ€Ñ‹</span>
                      ...
                      {order.discount > 0 && (
                        <div className="flex justify-between gap-6 text-green-700 dark:text-green-400">
                          <span>Ð¡ÐºÐ¸Ð´ÐºÐ°{order.promoCode ? ` (${order.promoCode})` : ''}</span>
                          <span>âˆ’{formatEuro(order.discount, locale)}</span>
                        </div>
                      )}
                      <div className="flex justify-between gap-6">
                        <span className="text-muted-foreground">Ð”Ð¾ÑÑ‚Ð°Ð²ÐºÐ°</span>
                        <span className="text-foreground">
                          {order.delivery === 0 ? 'Ð‘ÐµÑÐ¿Ð»Ð°Ñ‚Ð½Ð¾' : formatEuro(order.delivery, locale)}
                        </span>
                      </div>
                      {order.tax > 0 && (
                        <div className="flex justify-between gap-6">
                          <span className="text-muted-foreground">ÐÐ°Ð»Ð¾Ð³ (ÐÐ”Ð¡)</span>
                  ...
                  <div className="pt-2 border-t border-border">
                    <p className="text-sm font-semibold text-foreground mb-2">Ð˜Ð·Ð¼ÐµÐ½Ð¸Ñ‚ÑŒ ÑÑ‚Ð°Ñ‚ÑƒÑ</p>
                    <div className="flex flex-wrap gap-2">
                      {STATUS_LIST.map((s) => (
                        <Button
                          key={s}
                  ...
                  <div className="pt-2 border-t border-border">
                    <p className="text-sm font-semibold text-foreground mb-2">
                      ...
                      <span className="ml-2 text-xs font-normal text-gray-400 dark:text-gray-500">â€” ÐºÐ»Ð¸ÐµÐ½Ñ‚ Ð½Ðµ Ð²Ð¸Ð´Ð¸Ñ‚</span>
                    </p>
                    <textarea
                      rows={3}
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      {getOrderNote(order.id) && noteDrafts[order.id] === undefined && (
                        <span className="text-xs text-gray-400 dark:text-gray-500">Ð—Ð°Ð¼ÐµÑ‚ÐºÐ° ÑÐ¾Ñ…Ñ€Ð°Ð½ÐµÐ½Ð°</span>
                      )}
                    </div>
                  </div>
                </div>
        ...
        {filtered.length === 0 && (
          <div className="rounded-xl border border-border p-10 bg-muted text-center text-sm text-muted-foreground">
            {orders.length === 0 ? 'Ð—Ð°ÐºÐ°Ð·Ð¾Ð² Ð¿Ð¾ÐºÐ° Ð½ÐµÑ‚' : 'ÐÐµÑ‚ Ð·Ð°ÐºÐ°Ð·Ð¾Ð² Ð¿Ð¾ Ð²Ñ‹Ð±Ñ€Ð°Ð½Ð½Ñ‹Ð¼ Ñ„Ð¸Ð»ÑŒÑ‚Ñ€Ð°Ð¼'}
          </div>
        )}
      </div>
  </>
